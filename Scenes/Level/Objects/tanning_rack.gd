...
@export var node_name:String = "tanning_rack"
...
func rotate_item(side):
	...
	match side:
		"front":
			...
			%Sprite.set_region_rect(Rect2(0,0,32,32))
			%Sprite.flip_h = false
			%Sprite.position = Vector2(16,0)
			%Sprite.z_index = 0
			%HeroPlacement.position = Vector2(16,24)
			#Placement du marker
			$Marker2D.position = Vector2(16,24)
		"right_side":
			%CollisionShape2D.shape.size = Vector2(32,16)
			%CollisionShape2D.position = Vector2(16,8)
			%Sprite.set_region_rect(Rect2(64,0,16,32))
			%Sprite.flip_h = false
