...
@export var node_name:String = "workbench"
...
func rotate_item(side):
	...
	match side:
		"front":
			...
			%CollisionShape2D.shape.size = Vector2(32,48)
			%CollisionShape2D.position = Vector2(16,24)
			#choix et mise en place du sprite
			%Sprite.set_region_rect(Rect2(0,0,32,48))
			%Sprite.flip_h = false
		...
		"right_side":
			...
			%Sprite.set_region_rect(Rect2(64,0,32,48))
			%Sprite.flip_h = false
			%Sprite.position = Vector2(16,8)
			%Sprite.z_index = 0
		...
		"back":
			...
			%Sprite.set_region_rect(Rect2(32,0,32,48))
			%Sprite.flip_h = false
			%Sprite.position = Vector2(16,8)
			%Sprite.z_index = 1
			%HeroPlacement.position = Vector2(16,8)
			$Marker2D.position = Vector2(16,16)
		"left_side":
			%CollisionShape2D.shape.size = Vector2(48,32)
			%CollisionShape2D.position = Vector2(24,16)
			%Sprite.set_region_rect(Rect2(64,0,32,48))
			%Sprite.flip_h = true
			%Sprite.position = Vector2(32,8)
			%Sprite.z_index = 0
