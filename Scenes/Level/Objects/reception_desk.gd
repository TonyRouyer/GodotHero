...
@export var node_name:String = "reception_desk"
...
func rotate_item(side):
	...
	match side:
		...
		"back":
			...
			%CollisionShape2D.position = Vector2(32,16)
			#choix et mise en place du sprite
			%Sprite.set_region_rect(Rect2(64,0,64,48))
			%Sprite.flip_h = false
			%Sprite.position = Vector2(32,8)
			%Sprite.z_index = 0
			%HeroPlacement.position = Vector2(32,24)
		...
		"left_side":
			...
			%Sprite.z_index = 1
			%Sprite.position = Vector2(16,24)
			%HeroPlacement.position = Vector2(24,24)
